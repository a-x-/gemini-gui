...
function filterBrowsers(browsersFromCli, browsersFromConfig) {
    if (!browsersFromCli) {
        return browsersFromConfig;
    }

    var unknownBrowsers = _.difference(browsersFromCli, browsersFromConfig),
        validBrowsers = _.intersection(browsersFromCli, browsersFromConfig);

    unknownBrowsers.forEach(function(browserId) {
    ...
    return _.isEmpty(validBrowsers) ? browsersFromConfig : validBrowsers;
}
...
function App(options) {
    ...
    this._browserIds = filterBrowsers(this._options.browser, this._gemini.browserIds);

    this.referenceDirs = _.zipObject(
        this._browserIds,
        this._browserIds.map(function(browserId) {
...
App.prototype._readTests = function() {
    ...
    return this._gemini.readTests(this._options.testFiles, this._options.grep)
        ...
        .then(redefineBrowsersForSuites_)
        .then(function(suites) {
            _this._tests = new Tests(_this, suites);
        });

    function redefineBrowsersForSuites_(suites) {
        return suites.map(function(suite) {
            return _.extend(suite, { browsers: _this._browserIds });
        });
    }
};
